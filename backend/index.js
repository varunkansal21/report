...
app.post('/addEmployee',(req, res) => {
 ...
 console.log(doj);
  ...
  let data= {id,name,doj,dol,mobile};
...
// Find number from email
app.post('/findNumber',(req,res)=>{
    let id=req.body.id;
  console.log(id);
    const sql=con.query("SELECT MOBILE FROM EMPLOYEE WHERE ID = ?",[id],async(err,result)=>{
      if(err ){
        console.log(err);
        res.send(JSON.stringify({"status":false}));
        return;
      }
      res.send(JSON.stringify({"status":true,"response":result}));
    });
});
